...
void LOG(std::wstring_view format_str, Args&&... args) {
	auto str = std::vformat(format_str, std::make_wformat_args(std::forward<Args>(args)...));
	PERSIST(str);
	_globalCvarManager->log(std::move(str));
}
...
void DEBUGLOG(const FormatString& format_str, Args&&... args) {
	if constexpr (DEBUG_LOG) {
		auto str = std::format("{} {}", std::vformat(format_str.str, std::make_format_args(std::forward<Args>(args)...)), format_str.GetLocation());
		PERSIST(str);
		_globalCvarManager->log(std::move(str));
	}
}
...
void DEBUGLOG(const FormatWstring& format_str, Args&&... args) {
	if constexpr (DEBUG_LOG) {
		auto str = std::format(L"{} {}", std::vformat(format_str.str, std::make_wformat_args(std::forward<Args>(args)...)), format_str.GetLocation());
		PERSIST(str);
		_globalCvarManager->log(std::move(str));
	}
}
